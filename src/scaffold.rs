...
Two modes:
  - Flags only, with an <AppName>: non-interactive, never touches a
    database — only ever writes files. Good for scripts/CI.
  - No <AppName> (or --create): interactive, create-react-app style —
    prompts for whatever's missing (app name, database URL, theme,
    single file vs. directory), then connects to that database and
    syncs the new app immediately, so it's ready to `cargo run` as
    soon as the prompts finish.
...
fn scaffold_file(target: &str, name: &str, theme: &str) -> Result<()> {
...
# file, synced straight into Postgres. Run it with:
#
...
fn scaffold_dir(target: &str, name: &str, theme: &str) -> Result<()> {
    refuse_to_overwrite(target)?;
...
# they would inside a single file. Run it with:
#
#   export DATABASE_URL=postgres://postgres:postgres@localhost:5432/pgapp
#   cargo run -- {target}
